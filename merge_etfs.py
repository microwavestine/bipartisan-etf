        ...
        for row in reader:
            ...
            if not (number_filter) and (
                ...
                if re.search(" {2,}", row[0]):
                    ...
                    row[0] = row[0].strip()
                    #print(row)
                    if ETFCode == "NANC": NANC_STOCKS.add(row[0])
                    if ETFCode == "KRUZ": KRUZ_STOCKS.add(row[0])
                if ETFCode == "NANC": NANC_STOCKS.add(row[0])
                if ETFCode == "KRUZ": KRUZ_STOCKS.add(row[0])


analyzeCSV(NANC_CSV_FILE, "NANC")
analyzeCSV(KRUZ_CSV_FILE, "KRUZ")
